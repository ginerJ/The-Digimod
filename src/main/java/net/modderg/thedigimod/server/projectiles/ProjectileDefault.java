...
import net.modderg.thedigimod.server.projectiles.effects.ProjectileEffect;
...
public class ProjectileDefault extends AbstractArrow implements GeoEntity {
    ...
    protected void shootAtTargetWithSpeed(DigimonEntity mob, float speed) {
    ...
    protected void onHitEntity(EntityHitResult hitted) {
        ...
        if(hurtNotDigimonTarget(hitted.getEntity())){
            if(this.getOwner() instanceof DigimonEntity cd && hitted.getEntity() instanceof LivingEntity lv && !cd.isOwnedBy(lv)){
    ...
    protected int getDmgPlusEffects(DigimonEntity cd, DigimonEntity hcd) {
